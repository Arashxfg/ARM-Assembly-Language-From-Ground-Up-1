...
&nbsp;&nbsp;&nbsp;Some processor cores like cortex-M3 and cortex-M4 only execute thr `THUMB-2` instruction, therefore there are no `ARM` instructions at all.  
...
&nbsp;&nbsp;&nbsp;In this lesson we'll see how to change the contents of registers using boolean algebra. To be certain, we are going to use the `EXCLUSIVE OR` instruction to swap the contents od registers.

[code_4](../code_files/4/main.s)
